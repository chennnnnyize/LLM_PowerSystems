...
for data_dir in data_dirs:
    ...
    for model_name in model_names:
        print(f"Training {model_name} on {data_dir}")
        model = ImageClassifier(model_name).model.to(device)
        train(model, model_name, dataloader)
        results[data_dir][model_name] = test(model, dataloader)
        torch.save(model.state_dict(), f"models/{model_name}_{data_dir.split('/')[-1]}.pt")
